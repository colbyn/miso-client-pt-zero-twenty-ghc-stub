...
-- ~
import Core hiding (map, null, toLower, toTitle, toUpper)
...
import Data.JSString.Internal.Fusion.CaseMapping
  (foldMapping, lowerMapping, titleMapping, upperMapping)
...
cons !w (Stream next0 s0) = Stream next (C1 s0)
    where
...
snoc (Stream next0 xs0) w = Stream next (J xs0)
  where
...
append (Stream next0 s01) (Stream next1 s02) =
    Stream next (L s01)
    where
...
uncons (Stream next s0) = loop_uncons s0
    where
      loop_uncons !s = case next s of
                         Yield x s1 -> Just (x, Stream next s1)
...
tail (Stream next0 s0) = Stream next (C0 s0)
    where
      next (C0 s) = case next0 s of
...
init (Stream next0 s0) = Stream next (Init0 s0)
    where
...
compareLengthI (Stream next s0) n = loop_cmp 0 s0
...
isSingleton (Stream next s0) = loop 0 s0
    where
...
intersperse c (Stream next0 s0) = Stream next (I1 s0) --  len
    where
...
toTitle (Stream next0 s0) = Stream next (CC (False :*: s0) '\0' '\0')
  where
...
justifyLeftI k c (Stream next0 s0) =
    Stream next (s0 :*: S1 :*: 0)
  where
    next (s :*: S1 :*: n) =
        case next0 s of
...
any p (Stream next0 s0) = loop_any s0
    where
...
all p (Stream next0 s0) = loop_all s0
    where
...
maximum (Stream next0 s0) = loop0_maximum s0
    where
...
minimum (Stream next0 s0) = loop0_minimum s0
    where
...
scanl f z0 (Stream next0 s0) = Stream next (S1 :*: z0 :*: s0)
  where
    {-# INLINE next #-}
...
replicateCharI n c
    | n < 0     = empty
    | otherwise = Stream next 0 -- HINT maybe too low
  where
    next i | i >= n    = Done
...
isPrefixOf (Stream next1 s1) (Stream next2 s2) = loop (next1 s1) (next2 s2)
    where
      loop Done      _ = True
      loop _    Done = False
...
elem w (Stream next s0) = loop_elem s0
    where
...
findBy p (Stream next s0) = loop_find s0
    where
...
findIndicesI p (Stream next s0) = loop_findIndex 0 s0
  where
...
countCharI a (Stream next s0) = loop 0 s0
  where
